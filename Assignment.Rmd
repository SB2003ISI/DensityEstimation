...
\vspace{5pt}
\par
\textbf{Keywords:} Cumulant Generating Function, Edgeworth Expansions, Hermite Polynomials, Kernel Density Estimation, Kullback-Leibler Divergence, Lagrange Polynomials, Saddle Point Approximation. 
\end{mdframed}
...
```{r,echo=F,fig.cap="PDF of $U$ and it's approximated density through Saddlepoint Approximation 2",fig.height=2.8}
options(warn=-1)
# Compute densities
...
# What happens when $\boldsymbol{n}$ is large ?

Remember, \eqref{eq:density} can be considered as the density of sample range of $n+1$ number of standard Exponential samples. Consequently, as $n$ gets larger, the sample ranges will also be larger and can be expected to be rightly skewed. To assess how the above mentioned approximations and KDE performs we have calculated the Kullback-Leibler divergence, $L_2$ and $L_{\infty}$ distances between the exact density \eqref{eq:density} and the approximations for different values of $n$.

The Kullback-Leibler divergence between $f$ w.r.t. the true density $g$ has been defines as,$$D_{KL}\left(\left.f\right\Vert g\right)\coloneqq \int_{x\in\mathrm{supp}\left(g\right)}\ln\left[\frac{f\left(x\right)}{g\left(x\right)}\right]f\left(x\right)dx$$

and, $L_p$ and $L_\infty$ norm is defined by,$$\begin{aligned}\left\Vert f-g\right\Vert_p&\coloneqq\left(\int_\mathbb{R}\left|f(x)-g(x)\right|^pdx\right)^{\frac{1}{p}}\\
\left\Vert f-g\right\Vert_\infty &\coloneqq\underset{x\in\mathbb{R}}{\sup}\left|f(x)-g(x)\right|
\end{aligned}$$respectively. The following tables shows the outcomes.
...
df=data.frame(n_vec,results$KL_Divergence %>% round(.,5))
colnames(df) <- c("\\textbf{$n$}", 
                  "\\textbf{Div. \\eqref{eq:kern}}", 
                  "\\textbf{Div. \\eqref{eq:edge}}", 
                  "\\textbf{Div. \\eqref{eq:saddleI}}", 
                  "\\textbf{Div. \\eqref{eq:saddleII}}")
rownames(df)=NULL
kable(df,
      format = "latex", 
      escape = FALSE, 
      booktabs = TRUE,
      caption = "KL Divergence of Approximations. \\label{tab:KL}",
      align = "c") %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
df=data.frame(n_vec,results_l2 %>% round(.,5))
colnames(df) <- c("\\textbf{$n$}", 
                  "\\textbf{Dist. \\eqref{eq:kern}}", 
                  "\\textbf{Dist. \\eqref{eq:edge}}", 
                  "\\textbf{Dist. \\eqref{eq:saddleI}}", 
                  "\\textbf{Dist. \\eqref{eq:saddleII}}")
rownames(df)=NULL
kable(df,
      format = "latex", 
      escape = FALSE, 
      booktabs = TRUE,
      caption = "$L_{2}$ distances between Approximations and $f_{U}$. \\label{tab:L2}",
      align = "c") %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
df=data.frame(n_vec,results_inf %>% round(.,5))
colnames(df) <- c("\\textbf{$n$}", 
                  "\\textbf{Dist. \\eqref{eq:kern}}", 
                  "\\textbf{Dist. \\eqref{eq:edge}}", 
                  "\\textbf{Dist. \\eqref{eq:saddleI}}", 
...
kable(df,
      ...
      escape = F, 
      booktabs = TRUE,
      caption = "$L_{\\infty}$ distances between Approximations and $f_{U}$. \\label{tab:Linf}",
      align = "c") %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
In Table \ref{tab:KL}, \ref{tab:L2} and \ref{tab:Linf} we see that the distances corresponding to \eqref{eq:saddleI} and \eqref{eq:saddleII} keeps on increasing with $n$, whereas \eqref{eq:kern} and \eqref{eq:edge} gives a stable outcome. That's why, though for small $n$ saddle point approximations outperformed Edgeworth Expansions, in general saddle point approach is not recommended.
