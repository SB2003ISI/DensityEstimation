...
params:
  name: "Sourav Biswas"
  roll: "MD2320"
  email: "sourav3biswas2003@gmail.com"
---

```{=latex}
\begin{center}
{\Large\textbf{Frontiers in Density Estimation: An Unified Study of Kernel, Edgeworth, and Saddle Point Approaches}}\\[1em]
...
\footnotesize
Roll: `r params$roll`\\
$^*$ \Letter{} E-mail: \href{mailto:`r params$email`}{`r params$email`}
\end{center}
```
...
\vspace{5pt}  % Small vertical space
\par \textbf{Keywords:} Cumulant Generating Function, Edgeworth Expansions, Kernel Density Estimation, Lagrange Polynomials, Saddle Point Approximation. 
\end{abstract}
...
\end{aligned}
$$ Say, $U=X_{\left(n+1\right)}-X_{\left(1\right)},V=X_{(1)}$. Which immediately implies, $X_{(n+1)}=U+V,X_{(1)}=V$. By making the transform from $\left(X_{(1)},X_{(n+1)}\right)$ to $\left(U,V\right)$, we get the jacobian of the tranformation as,$$
...
We have taken a sample of size $500$ by generating samples using \autoref{thm:convexp} for the estimation procedure with bandwidth selected through LSCV. The estimated density looks like this,
...
H_k\left(u\right)=\left(-1\right)^ke^{\frac{1}{2}u^2}\frac{d^k}{du^k}e^{-\frac{1}{2}u^2}.
$$

Consequently from \eqref{eq:sden}, density of $S_n$ can be estimated through,\begin{equation}\label{eq:edge}
...
Therefore, if cumulants up to the third order are available, \eqref{eq:density} can be accurately approximated by \eqref{eq:edge}. Moreover, in the presence of a sample, the corresponding sample cumulants may be substituted into \eqref{eq:edge}.
...
```{r,echo=F,fig.cap="PDF of $U$ and it's estimated density through Edgeworth Expansions"}
...
p <- ggplot() +
  ...
  scale_color_manual(name = "Density Type",
                     values = c("Edgeworth Expansion" = "purple",
                                "Exact Density" = "#D55E00")) +
  labs(title = "Estimation through Edgeworth Expansions",
       x = "x", y = "Density") +
  theme_minimal(base_size = 14) +
  theme(legend.position = "top")
...
This equation need to be solved numerically. The saddle point approximated density looks like,
...
```{r,echo=F,fig.cap="PDF of $U$ and it's estimated density through Saddlepoint Approximation 1"}
options(warn=-1)
...
p <- ggplot() +
  ...
  scale_color_manual(name = "Density Type",
                     values = c("Saddlepoint Approximation 1" = "lightgreen", 
                                "Exact Density" = "#D55E00")) +
  labs(title = "Estimation through Saddlepoint Approximation 1",
       x = "x", y = "Density") +
...
p <- ggplot() +
  ...
  scale_color_manual(name = "Density Type",
                     values = c("Saddlepoint Approximation 2" = "tomato", 
                                "Exact Density" = "#D55E00")) +
  labs(title = "Estimation through Saddlepoint Approximation 2",
       x = "x", y = "Density") +
  theme_minimal(base_size = 14) +
  theme(legend.position = "top",
