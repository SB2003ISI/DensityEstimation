...
```{=latex}
\begin{center}
{\Large\textbf{Beyond the Basics: Advanced Density Estimation with Kernel, Edgeworth, and Saddle Point Approaches}}\\[1em]
...
Note that, integration of the above identity of the Hermite polynomial gives,$$\begin{aligned}\int_{-\infty}^u\phi\left(x\right)H_k\left(x\right)dx&=\left(-1\right)^k\int_{-\infty}^ud\left(\frac{d^{k-1}}{dx^{k-1}}\phi\left(x\right)\right)\\[1mm]&=\left(-1\right)^{k}\frac{d^{k-1}}{du^{k-1}}\phi\left(u\right)\\[1mm]&=-\phi\left(u\right)H_{k-1}\left(u\right).\end{aligned}$$

Which can be used to get $H_n\left(u\right)$'s recursively.
